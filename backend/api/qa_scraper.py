...
def main(full_url):
    ...
    formatted_data = [
        (
            row[0],  # row[0] corresponds to the "url" column
            row[3],  # row[1] corresponds to the "embeddings" column
            {"url":row[0], "text": row[1], "n_tokens": row[2]}  # row[2] corresponds to the "text" column, row[3] corresponds to the "n_tokens" column
        )
        for row in df.itertuples(index=False)
    ]
    print('Uploading data to Pinecone...')
    
    # Retry loop for upserting the data
    max_attempts = 10
